...
impl Particles {
    ...
    /// Smoke + fire behind a rocket travelling from `from` to `to` (metres) this frame.
    pub fn rocket_trail(&mut self, from: Vec3, to: Vec3, dir: Vec3, dt: f32) {
        let tail = -dir * 18.0 * UNIT;
        // Smoke: ~70 puffs per second spread along the path the rocket covered this frame.
        let n = (70.0 * dt).ceil().max(1.0) as usize;
        for k in 0..n {
            let s = (k as f32 + self.rand()) / n as f32;
            let p = from.lerp(to, s) + tail + self.sphere() * 3.0 * UNIT;
            let frame = self.frame_for(Sprite::Smokelit);
            let life = self.range(0.9, 1.4);
            let vel = self.sphere() * 25.0 * UNIT + Vec3::Y * 12.0 * UNIT;
            let particle = Particle {
                ...
                size0: 8.0 * UNIT,
                size1: 34.0 * UNIT,
                color: Vec3::new(0.78, 0.78, 0.78),
                alpha: 0.5,
                fade_in: 0.05,
                fade_out: 0.6,
                rot: self.range(0.0, std::f32::consts::TAU),
                spin: self.range(-1.5, 1.5),
                gravity: 0.0,
                drag: 1.5,
            };
            self.list.push(particle);
        }
        // Fire: short-lived additive glows at the nozzle (the trail's `rockettrail_fire` child).
        for _ in 0..2 {
            let p = to + tail * 0.6 + self.sphere() * 2.0 * UNIT;
            let hot = self.rand();
            let particle = Particle {
                sprite: if hot > 0.5 { Sprite::Glow } else { Sprite::Softglow },
                frame: 0,
                pos: p,
                vel: -dir * 40.0 * UNIT,
                age: 0.0,
                life: self.range(0.05, 0.1),
                size0: self.range(12.0, 18.0) * UNIT,
                size1: 6.0 * UNIT,
                color: Vec3::new(1.0, 0.55 + 0.3 * hot, 0.15),
                alpha: 0.9,
                fade_in: 0.0,
                fade_out: 0.7,
                rot: self.range(0.0, std::f32::consts::TAU),
...
fn update_particles(
    ...
    for p in &particles.list {
        ...
        let a = p.alpha_now();
        let col = [p.color.x, p.color.y, p.color.z, a];
